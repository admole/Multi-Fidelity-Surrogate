...
def mfgp(x_lf, lf, x_hf, hf):
    ...
    gpr_lf = GaussianProcessRegressor(kernel=RBF(), n_restarts_optimizer=200).fit(x_lf, lf)

    l1mean = gpr_lf.predict(x_hf)
    l2_train = np.hstack((x_hf, l1mean))
    gpr_mf_l2 = GaussianProcessRegressor(kernel=RBF() * RBF() + RBF(), n_restarts_optimizer=200).fit(l2_train, hf)
